...
/*
 * 추가되어야 하는 모듈
 * set_stripe_num(stripe id)
 * get_stripe_num(Address)
 * */
...

{
	...
	this->parent.parent.parent.parent.controller.ftl->bm->cost_insert(this);
...
enum status Block::write(Event &event)
{
	...
	if(event.get_noop() == false)
	{
		...
		this->parent.parent.parent.parent.controller.ftl->bm->update_block(this);
...
enum status Block::_erase(Event &event)
{
	...
	if (!event.get_noop())
	{
		...
		printf("\n \t\t\t\ Block::_erase erase_delay =  %5.20lf\n", erase_delay);
