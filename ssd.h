...
/* ssd.h
 * HooYoung Ahn 2015-10-12
 * Main SSD header file
 * 	Lists definitions of all classes, structures,
 * 		typedefs, and constants used in ssd namespace
 *		Controls options, such as debug asserts and test code insertions
 *	- we modified existing ssd.h of Brendan Tauras to simulate multiple independent SSDs
 *	- key change is that we modified static variables of the SSD class to non-static
 *	- RAIDSsd class has method to process events of different RAID levels
 * - Stripe class has configurations of stripe information of RAID architectures
 */
...
public:
  ...
  void instance_initialize(FtlParent *ftl);
	 Block_manager *inst;
...
public:
	...
	ssd::Block_manager *instance_initialize_BlockManager();
	ssd::Block_manager *get_BlockManager();
	ssd::Block_manager *bm;
...
public:
	...
	//for parity write (PN is the SSD # for parity)
	double event_arrive_RAID5(enum event_type type, ssd::ulong logical_address, uint size, double start_time,ssd::uint DN);
	double event_arrive_RAID5_parityWrite(enum event_type type, ssd::ulong logical_address, uint size, double start_time,ssd::uint DN);
...
{
public:
	Stripe(uint stripe_size = STRIPE_SIZE);
	~Stripe(void);
	void printStripestats();
	std::vector<ssd::ulong> Vaddrs;
	bool parityExist;
	ssd::ulong stripeID;
	ssd::ulong dataUpdateCnt;
	ssd::ulong parityUpdateCnt;
...
class RaidwithParity
{
public:
	RaidwithParity (uint ssd_size = SSD_SIZE);
	~RaidwithParity(void);
	double event_arrive(enum event_type type, ulong logical_address, uint size, double start_time);
	double event_arrive(enum event_type type, ulong logical_address, uint size, double start_time, void *buffer);
	void *get_result_buffer();
