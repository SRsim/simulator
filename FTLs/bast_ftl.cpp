	...
	FtlParent(controller)
{
	...
	printf("# of addressable blocks = %lu \n", NUMBER_OF_ADDRESSABLE_BLOCKS);
...
if (log_map.find(lba) == log_map.end())
	{
	//printf("\n log_map.find(lba) == log_map.end() \n");
		allocate_new_logblock(logBlock, lba, event);
	//	logBlock->address.print();
	}
...
{
	//printf("@@@ numValid < BLOCK_SIZE numValid = %lu \n", numValid);
	...
	//printf("\n == replaced address == \n");
	Address replaced_addr =event.get_address();
	//replaced_addr.print();
}

else {
		//numValid > BLOCK_SIZE
	//printf("numValid >= BLOCK_SIZE numValid = %lu  log block address = \n", numValid);
	//logBlock->address.print();
...
if (data_list[lba] != -1)
{
 //	printf("\n FtlImpl_Bast::write data_list[lba] != -1 \n");
...
enum status FtlImpl_Bast::trim(Event &event)
{
				...
				//	printf(" \nFtlImpl_Bast::trim eventAddress = %lu \n", eventAddress.real_address);
					...
					if (logBlock != NULL && logBlock->pages[eventAddress.page] != -1) // If page is in the log block
					{
						...
						if (lBlock->get_state() == INACTIVE) // All pages invalid, force an erase. PTRIM style.
						{
							...
							//this->bm->erase_and_invalidate(event, returnAddress, LOG);
							controller.ftl->bm->erase_and_invalidate(event, returnAddress, LOG);
				}
					...
					if (data_list[lookupBlock] != -1) // Datablock
					{
						...
						if (dBlock->get_state() == INACTIVE) // All pages invalid, force an erase. PTRIM style.
						{
							...
							//this->bm->erase_and_invalidate(event, dataAddress, DATA);
							controller.ftl->bm->erase_and_invalidate(event, dataAddress, DATA);

						}
					}


					//parity log block erase
					if (data_list[lookupBlock] == -1 || (logBlock == NULL && logBlock->pages[eventAddress.page] == -1))
					{
						//printf("\n==FtlImpl_Bast::trim pairty LOG block Erase==\n");
...
void FtlImpl_Bast::allocate_new_logblock(LogPageBlock *logBlock, long lba, Event &event)
{
	if (log_map.size() >= BAST_LOG_PAGE_LIMIT)
		...
		//logBlock->address = this->bm->get_free_block(LOG, event);
		logBlock->address = 	this->controller.ftl->bm->get_free_block(LOG, event);
...
*/
	...
	if (isSequential)
	{
		...
		//this->bm->promote_block(DATA);
		controller.ftl->bm->promote_block(DATA);


		// Add to empty list i.e. switch without erasing the datablock.
		if (data_list[lba] != -1)
		{
			//printf("\n FtlImpl_Bast::isSequential data_list[lba] != -1 __2 \n");
			Address a = Address(data_list[lba], PAGE);

			//Block_manager::instance()->erase_and_invalidate(event, a, DATA);
			//this->bm->erase_and_invalidate(event, a, DATA);
			controller.ftl->bm->erase_and_invalidate(event, a, DATA);
			//printf("\n \t FtlImpl_Bast::isSequential SWITCH event.get_time_taken() = %5.20lf \n",event.get_time_taken());
		}
...
bool FtlImpl_Bast::random_merge(LogPageBlock *logBlock, long lba, Event &event)
{
	...
	Address eventAddress = Address(event.get_logical_address(), PAGE);
	//Address newDataBlock = this->bm->get_free_block(DATA, event);
	Address newDataBlock = controller.ftl->bm->get_free_block(DATA, event);
...
//printf("\nFtlImpl_Bast::random_merge erase_and_invalidate \n");
//this->bm->erase_and_invalidate(event, logBlock->address, LOG);
controller.ftl->bm->erase_and_invalidate(event, logBlock->address, LOG);
//printf("\ninvalidated Data block address \n");
//logBlock->address.print();


if (data_list[lba] != -1)
{
	//printf("\n ****************** invalidated LOG block address \n");
	Address a = Address(data_list[lba], PAGE);
	//this->bm->erase_and_invalidate(event, a, DATA);
	controller.ftl->bm->erase_and_invalidate(event,a, DATA);
	//a.print();
...
void FtlImpl_Bast::print_ftl_statistics()
{
	...
	//this->bm->print_statistics();
	controller.ftl->bm->print_statistics();
}
