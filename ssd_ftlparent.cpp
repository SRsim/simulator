...
FtlParent::FtlParent(Controller &controller) : controller(controller)
{
	...
	//bm = new ssd::Block_manager(this);
	bm = instance_initialize_BlockManager();

	printf("FtlParent::FtlParent Number of addressable blocks: %u\n", NUMBER_OF_ADDRESSABLE_BLOCKS);

}


ssd::Block_manager* FtlParent::instance_initialize_BlockManager()
{

	this->bm = new ssd::Block_manager(this);
 return bm;
}

ssd::Block_manager* FtlParent::get_BlockManager()
{
 return bm;
}
